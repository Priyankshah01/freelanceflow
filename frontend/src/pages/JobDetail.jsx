...
const JobDetail = () => {
  ...
  const handleApplicationSubmit = async (e) => {
    ...
    if (!applicationData.coverLetter.trim() || !applicationData.bidAmount || !applicationData.timeline) {
    ...
    try {
      // TODO: Replace with actual proposal API endpoint
      ...
      showMessage('error', `Failed to submit application: ${error.message}`);
    } finally {
  ...
  const isOwnJob = user?.role === 'client' && job.client._id === user._id;
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {(job.requirements?.length > 0 || job.deliverables?.length > 0) && (
            ...
            {isOwnJob && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="text-center">
                  ...
                  <p className="text-sm font-medium text-gray-900 mb-2">Your Job Post</p>
                  <p className="text-xs text-gray-500">You are the client for this project</p>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">{getCategoryLabel(job.category)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-500">Project Size</span>
                  <span className="text-sm font-medium text-gray-900">{getProjectSizeLabel(job.projectSize)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-500">Experience Level</span>
                  <span className="text-sm font-medium text-gray-900">{getExperienceLabel(job.experienceLevel)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-500">Budget Type</span>
                  <span className="text-sm font-medium text-gray-900 capitalize">{job.budget.type} Price</span>
                </div>
              ...
              <div className="flex items-start space-x-3 mb-4">
                ...
                <div>
                  ...
                  {job.client.profile?.company && (
                    <p className="text-sm text-gray-600">{job.client.profile.company}</p>
                  )}
                  <div className="flex items-center mt-1">
                    ...
                    <span className="text-xs text-gray-500">
                      Member since {new Date(job.client.createdAt).getFullYear()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {job.client.ratings?.count > 0 && (
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    {job.client.ratings.average.toFixed(1)} ({job.client.ratings.count} reviews)
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="p-6">
                ...
                <form onSubmit={handleApplicationSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <FormField
                      label={`Your Bid Amount ($${job.budget.type === 'hourly' ? '/hr' : ''})`}
                      type="number"
                      name="bidAmount"
                      ...
                      min="1"
                      required
                      placeholder={job.budget.type === 'fixed' ? job.budget.amount : job.budget.hourlyRate.min}
                    />
                    ...
                    <div className="flex">
                      ...
                      <div className="ml-3">
                        ...
                        <div className="mt-2 text-sm text-yellow-700">
                          <p>Make sure you understand the project requirements and can deliver within the specified timeline.</p>
                        </div>
                      </div>
                    </div>
                  </div>
